...
### Refresh requirements inside the virtual environment only
Use this when the repository is already up to date but the dependencies changed (for example after adding `requirements.txt`).
```bash
cd /srv/action-runtime/app/srvaction-runtime
source .venv/bin/activate
pip install --upgrade pip
